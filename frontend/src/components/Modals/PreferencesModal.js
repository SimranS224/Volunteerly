...
// runs only when component first mounted due to second argument []
  React.useEffect(() => {
    async function setUp(){
      ...
      let times_available = []
      // if logged in, then get custom preferences including event types and
      // time availability
      if(props.curUser){
          console.log("user is: ", props.curUser.id)
          const preferences = await userService.getPreferences(props.curUser.id)
          // now iterate over all of the event type preferences,
          // and check the ones that are true for this user
         ...
         // now find all of the dates when user is available. will be displayed
         // on the date selector as the times the user specified they are available 
      ...
      setState({...state, eventTypes:eventTypes, schedule: times_available})
  ...
  const updateHandler = name => event => {
    ...
    setState({ ...state, eventTypes: eventTypesCopy });
  };
  ...
  const saveCloseHandler = () => {
    // first create the preference state we will be sending to Redux Store to 
    // update the user preferences globally
    ...
    if(props.curUser){
      ...
      for(let i=0;i<state.eventTypes.length;i++){
        ...
        if(element.set){
          eventPrefForDb.push({
            "volunteer_id": props.curUser.id,
            "event_type_id": element.id
          })
        }
      }
      ...
      reduxPreferenceState.availability.forEach(element => {
        availabilityForDb.push({
          "volunteer_id": props.curUser.id,
      ...
      >
        ...
        <div className="root">
            <FormControl required component="fieldset" className="formControl">
              ...
              <FormGroup>
              <List>
                {state.eventTypes.map((event)=>{
                  return   <ListItem key={event.type}><FormControlLabel
                  control={<Checkbox checked={event.set} onChange={updateHandler(event.type)} value={event.type} />}
                  label={event.type}
                /></ListItem>
                })}
              </List>
