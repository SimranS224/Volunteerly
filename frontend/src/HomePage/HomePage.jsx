...
class HomePage extends React.Component {
    ...
    enrollUser = (event) => {
        ...
        userService
            .enrollInEvent(this.props.curUser.id, event.id)
            .then((res) => {
                if (res.statusCode !== 200) {
                    this.setState({showEnrollmentSuccess: true, dialog_open: false, selected: null})
                }
            });
    }

    handleClose = () => {
        this.setState({showEnrollmentSuccess: false})
    }
    ...
    render() {
        ...
        return (
            <div className="HomePage">
                <Snackbar
                    open={this.state.showEnrollmentSuccess}
                ...
                <div className="hero-section">
                    <div className="hero-container">
                        <div className="hero-content">
                            ...
                            <input
                                ref="query"
                                placeholder=" Search for volunteering opportunities (try: beach cleanup)"
                                type="text"
                        ...
                        {filtered.length > 0 && filtered.map((event, i) => {
                            return <ListItem key={'event' + i.toString()}>
                                <OppCard
                                    ...
                                    onClick={() => {
                                    this.setState({dialog_open: true, selected: i})
                                }}/>
                                <Dialog
                                    fullScreen
                                    open={this.state.dialog_open}
                                    onClose={() => {
                                    this.setState({dialog_open: false, selected: null})
                                }}
                                    ...
                                    <DialogContent>
                                        <Typography className="event-title" variant="h2">
                                            {this.state.selected !== null
                                                ? filtered[this.state.selected].name
                                                : null}
                                        </Typography>
                                        <Button
                                            className="primary-button"
                                            variant="contained"
                                            onClick={() => this.enrollUser(filtered[this.state.selected])}
                                            disableElevation>
                                            Enroll
                                        </Button>
                                        <Typography className="event-desc" variant="h4">
                                            {this.state.selected !== null
                                                ? filtered[this.state.selected].description
                                                : null}
                                        </Typography>
                                        <Typography className="event-date" variant="h4">
                                            Starts {this.state.selected !== null
                                                ? hdate.prettyPrint(filtered[this.state.selected].start_date, {showTime: true})
                                                : null}
                                            <br/>
                                            Ends {this.state.selected !== null
                                                ? hdate.prettyPrint(filtered[this.state.selected].end_date, {showTime: true})
                                                : null}
                                            <br/>
                                            Location:
                                        </Typography>
...
const mapStateToProps = state => {
    return {curUser: state.userReducer.curUser, events: state.userReducer.events}
}
