...
import {User, Event,Enrollment, Stat, StatCategory, Achievement, AchievementEarned} from "../../db/models"
...
const processStatistics = async (event_id, user_id) =>{
        // increase number of events attended
        // findOrCreate is used because new users will have no stats recorded
        const [user_stat2, created2] = await Stat.findOrCreate({
                            where: {
                                volunteer_id: user_id,
                                stat_category_id: '2'
                            }
        ...
        const [user_stat1, created1] = await Stat.findOrCreate({
                            where: {
                                volunteer_id: user_id,
                                stat_category_id: '1'
                            }
        ...
        const [user_stat3, created3] = await Stat.findOrCreate({
                            where: {
                                volunteer_id: user_id,
                                stat_category_id: '3'
                            }
...
const processAchievements = async (user_id, num_events, num_hours, num_organizations) => {
    ...
    for(let i = 0; i < new_achieved.length; i++){
        ...
        await AchievementEarned.findOrCreate({
                                                where: {volunteer_id: user_id, achievement_id: new_achieved[i].id}
