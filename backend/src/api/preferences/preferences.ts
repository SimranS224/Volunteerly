...
const getPreferences = async(req : Request, res : Response) => {
    ...
    try {
        const event_preference = await sequelize
            .sync()
            .then(() => VolunteerEventPreference.findAll({
                where: {
                    volunteer_id: user_id
                },
                include: [EventType]
            }));
        const availability = await sequelize
            .sync()
            .then(() => VolunteerAvailability.findAll({
                where: {
                    volunteer_id: user_id
                }
            }));
...
const updatePreferences = async(req : Request, res : Response) => {
    ...
    try {
        ...
        const volunteerEventPreference = await sequelize
            .sync()
            .then(() => VolunteerEventPreference.findAll({
                where: {
                    volunteer_id: user_id
        ...
        await volunteerEventPreference.forEach(preference => preference.destroy());

        const volunteerAvailability = await sequelize
            .sync()
            .then(() => VolunteerAvailability.findAll({
                where: {
                    volunteer_id: user_id
        ...
        await volunteerAvailability.forEach(availability => availability.destroy());

        // create the new preferences and availability
        const event_preference: { volunteer_id: number; event_type_id: number; }[] = []
        const availability: { id: any; volunteer_id: number; day_of_week: string; start_hour: number; end_hour: number; }[] = []
        for (let i = 0; i < data.event_preference.length; i++) {
            let ev_pref = data.event_preference[i]
            await VolunteerEventPreference
                .create(ev_pref)
                .then((new_ev_pref) => {
        ...
        const eventsWithValidType = await sequelize
            .sync()
            .then(() => Event.findAll({
                where: {
                    [Op.or]: eventTypes,
                    ...
                    if (daysOfTheWeek.indexOf(curWeekDay) > -1) {
                        for (let j = 0; j < daysOfTheWeekAvailbility.length; j++) {
                            if (curWeekDay === daysOfTheWeekAvailbility[j][0]) {
                                ...
                                if ((daysOfTheWeekAvailbility[j][1] <= event.start_time && event.start_time <= daysOfTheWeekAvailbility[j][2]) || (daysOfTheWeekAvailbility[j][1] <= event.start_time && event.end_time <= daysOfTheWeekAvailbility[j][2])) {
                                    return event;
                                }
                            }
                        }
